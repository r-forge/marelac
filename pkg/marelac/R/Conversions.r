...
# the van der Waals coefficients...
{
  if (! is.null(gas))
  {
    ## simple indexed vectors are 5 times faster than row name indexing in data frame
    ...
    #
    #Waals.b <- c( 0.03219,0.04267,0.07685,0.03985,0.1281, 0.05622,0.09213,0.08407,
    ...
    nam <- row.names(waalscoef)
    a <- waalscoef[gas == nam, "a"]
    b <- waalscoef[gas == nam, "b"]
    if (is.na(a)) stop(paste("do not have a and b values for the gas", gas))
  }
