...
\begin{itemize}
\item chemical and physical constants, e.g. atomic weights, gas
  constants.
...
We use it to estimate the surface of the earth; the true surface is
510072000 $\rm km^2$:
...
\code{viscosity} calculates the shear viscosity of water, in
...
Conversion to $\rm mmol m^{-3}$ can be done via:
...
\code{vapor} estimates the partial pessure of water vapor, divided by
...
measured 10 metres above sea level).
...
(i.e. for \code{p=0})\footnote{Before plotting, the negative numbers
...
\m also implements several thermodynamic properties of
...
of salinity and temperature. We first define a function that does that
...
\code{molvol} estimates the volume of one liter of a gas or the molar
