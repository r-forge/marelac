...
\item the thermophysical properties of the seawater, as from the
  ...
  derivation based on a gibbs funtion \citep{Feistel08,McDougall}.
\end{itemize}
...
\citep{Stumm1964}. Since then, the ratio of C:N:P ratio was widely
agreed, but there is still discussion about the average of O and
H. \citet{Anderson1995} proposed a new formula $\rm
C_{106}H_{175}O_{42}N_{16}P_{1}$ for marine plankton and by NMR
analysis also \citet{Hedges2002} found an elemental composition with
much less hydrogen and oxygen than in the original formula ($\rm
C_{106}H_{175-180}O_{35-40}N_{15–20}S_{0.3-0.5}$).

Function \code{redfield} can be used to simplify conversions between
the main elements of biomass. The default molar ratio can be displayed
by:
...
or, rescaled to nitrogen by:

<<>>=
redfield(1, "N")
@

I addition, it is also possible to convert this to mass units, how
many mass units are related to 2 mass units (e.g. mg) P:

<<>>=
redfield(2, "P", "mass")
@

Finally, mass percentage can be obtained by:
...
<<>>=
x <- redfield(1, "P", "mass", ratio=c(C=106, H=180, O=45, N=16, P=1))
x / sum(x)
@

Note however, that all these formulae are intended to approximate the
average biomass composition but that for specific observations large
differences are natural, depending on the involved species and their
physiological state.
