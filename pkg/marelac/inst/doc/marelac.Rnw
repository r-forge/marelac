...
@
  ...
  correct for inconsistencies due to local composition anomalies. The
  ...
  (i.e. for \code{p=0}).
  \footnote{Before plotting, the negative numbers in the salinity anomaly are
  converted to \code{NA} (not available).  In the data set, numbers not
  available are denoted with \code{-99}.}
