...
\title{
  \proglang{R}-package \m: utilities for the MArine, Riverine, Estuarine,
...
@
  ...
  which have no stable isotopes (except U, Th, Pa).
<<>>=
unlist(AtomicWeight)
AtomicWeight$H
...
\end{figure}
  ...
  ($cm^2 hour^{-1}$), for several species at given salinity (S) temperature (t)
...
\end{figure}
  ...
  ($0.01 g cm^{-1} sec^{-1}$). Valid for 0<t<30 and 0<S<36.
