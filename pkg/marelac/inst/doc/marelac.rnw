...
\subsection{Two salinity scales}
  \citep{Millero08} provide a function to derive absolute salinity (expressed
...
@
  ...
  These functions have as extra arguments the gauge pressure (\code{p}) and
  latitude (\code{lat}) and longitude (\code{lon}). When provided, they also
  correct for inconsistencies induces by local composition anomalies. The
  correction then makes use of a conversion table that estimates the salinity
  variations as a function of present-day local seawater composition.
  
  The correction factors are in a data set called \code{sw_sfac}, which are
  used in the conversion functions. Below we first convert from practical to
...
<<sfac>>
@
\caption{factor to convert salinity scales}
\label{fig:sfac}
\end{figure}
...
\subsection{Thermophysical seawater properties}
  ...
  Note that the formulations use the absolute salinity scale, \citep{Millero08}.
