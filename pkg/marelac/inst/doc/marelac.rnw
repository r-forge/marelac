...
@
\subsection{seawater properties}
Several sewater profperties, using \citep{Feistel08} or according to
the UNESCO 1983 polynomial \citep{Fofonoff83} are included. For instance,
the heat capacity:
<<>>=
sw_cp(S=40,t=1:20)
sw_cp(S=40,t=1:20,UNESCO=TRUE)
@
The dependency on salinity, and temperature for some of those is hereby plotted,
using a function
...
<<sw>>
@
\caption{seawater properties as a function of salinity and temperture - see text for R-code}
\label{fig:sw}
\end{figure}
