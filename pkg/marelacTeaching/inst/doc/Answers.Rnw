...
\normalsize
  The saturated oxygen concentration in water ($\mu mol kg^{-1}$), as
...
\normalsize
  \begin{itemize}
    ...
    \item	For a matrix A, x is an eigenvector, and $\mu$ the eigenvalue of
      a matrix A, if Ax =$\mu$x. Test it!
  \end{itemize}
<<>>=
...
\normalsize
  \begin{itemize}
    \item Estimate the pH at equilibrium with alkalinity 2300
      $\mu mol kg^{-1}$ and the current pCO2 of 360 ppm.
...
\large \emph{Chapter 9.1 interpolating wind data}
  Wind velocities are: 5,6,7,9,4,6,3,7,9 at time 0, 3, $\ldots$, 24 o'clock respectively.
  \begin{itemize}
...
plot(ll,pp,xlab= expression("light, microEinst"~ m^{-2}~s^{-1}),
